...
// ## Why this is chunked
//
// The free plan gives an Edge Function 2 seconds of CPU. A three-minute run is
// 10,800 ticks of deterministic
// simulation, which is well past that. So one run is verified across a chain of
// invocations: each claims the job,
// replays a slice, serialises the simulation state, and posts back to itself for
// the next slice.
//
// That is the reason SimState serialisation exists at all, and the reason
// replaySlice is a separate entry point from
// replay: the property that a log replayed in slices of any size produces
// byte-identical state to one replayed in a
// single pass is what makes this safe, and packages/sim has tests for exactly
// that.
//
// ## What is trusted
//
// Nothing from the client except the input frames. The score, kills, accuracy
// and medals are all recomputed from the
// replay, and the claimed summary is only ever compared against them. A claim
// that exceeds the replay is a rejection,
...
//
// A pg_cron reconciler (reconcile_verification_jobs) kicks this function when
// a job has sat idle, so a failed chain call
 ...
 * Ticks per slice.
 *
 * 6000 ticks is 100 seconds of simulation. Measured at roughly 0.6 s of CPU,
 * which leaves generous headroom under the
 * 2 s cap for the fetch, the decompression and the commit. Tunable by env var
 * so a slow region can be dialled down
 * without a redeploy.
 */
const SLICE_TICKS = Number(Deno.env.get('VERIFIER_SLICE_TICKS') ?? '6000');

/** Lease length for a claimed job. Longer than a slice takes, short enough
 * that a crashed invocation frees it soon. */
const LEASE_SECONDS = 30;

/** Ghost promotion threshold: a run in the top few of its board is worth
 * racing against. */
const GHOST_RANK_LIMIT = 10;

/**
 * The name this function is deployed under, which is where the next slice is
 * posted.
 *
 * `pnpm functions:deploy` deploys the esbuild bundle as
 * `verify-run-bundled`, because the unbundled source cannot resolve its
 * workspace imports under Deno. Posting to `verify-run` would reach nothing,
 * and every run longer than one slice would stall
 ...
 *
 * The authoritative formula. The client has its own estimate for display, and
 * this deliberately ignores it: an
...
/**
 * Resolve the SimContent for a run.
 *
 * The shared sim content resolver validates the map, mode and exact contentHash
...
async function rejectRunAndDeleteJob(
  ...
  const rejection = await supabase.rpc('reject_run', {
    p_run_id: runId,
    p_reason: reason,
    p_first_mismatch_tick: firstMismatchTick,
  });
  if (rejection.error) {
    throw new Error(`reject failed: ${rejection.error.message}`);
  }
  if ((rejection.data as { rejected?: boolean } | null)?.rejected) {
    await deleteVerificationJob(supabase, jobId);
  }
}
...
async function fetchLog(supabase: SupabaseClient, path: string): Promise<RunLog>
{
  ...
  /*
   * The path extension records whether the client compressed it. Sniffing the
   * gzip magic number would also work, but
   * the extension is written by the same code that chose the encoding, so it
   * cannot disagree.
   */
  if (path.endsWith('.gz')) {
...
function toBase64(bytes: Uint8Array): string {
  let binary = '';
  // Chunked: a spread over a 100 KB array overflows the argument limit in some
  // runtimes.
  const CHUNK = 8192;
...
async function chain(jobId: string, secret: string): Promise<void> {
  ...
  /*
   * Deliberately not awaited for its result beyond dispatch: this invocation has
   * done its slice, and waiting for the
   * whole remaining chain would reintroduce the CPU limit this design exists to
   * avoid.
   */
  await fetch(url, {
    method: 'POST',
...
Deno.serve(async (req) => {
  ...
  /*
   * Three payload shapes. A Database Webhook on runs INSERT sends { record },
   * a chained call sends { job_id }, and the
   * reconciler sends {}. All three go on to claim, so the first slice needs no
  ...
  /*
   * Claim atomically. A conditional UPDATE rather than a select-then-update:
  ...
  if (!job) {
    // Nothing claimable. Not an error: another invocation has it, or the queue is
    // empty.
...
${runResult.error.message}`);
    ...
    const slice = replaySlice({
      ...
      // Checked after restore, so a serialisation defect is reported as itself rather
      // than as a replay mismatch.
      expectedResumeHash: (job.resume_hash as string | null) ?? undefined,
    });

    // A checkpoint disagreed. The tick is recorded so an honest bug can be
    ...
    if (!slice.done) {
      ...
      /*
       * Budget check. Without it a malformed or adversarial log could keep the chain
       * alive indefinitely and consume the
      ...
      await supabase
        ...
        .update({
          ...
          // The hash the NEXT slice checks after restoring, which is how a bad round trip
          // is caught early.
          resume_hash: slice.checkpoints.at(-1)?.hash ?? null,
          slices_done: slicesDone,
          // Release the lease immediately: the chained call should be able to claim at
          // once.
          claimed_until: null,
          last_error: null,
        })
     ...
     *
     * A claim BELOW the replay is tolerated: a client that under-reports has not
     * gained anything, and the verified
     * figure is what gets used regardless. A claim ABOVE it is rejected, because
     * that is exactly what a tampered
     * client produces.
     */
    ...
    // The final state hash must also agree, which catches a log whose checkpoints
    // pass but whose end state differs.
    ...
    /*
     * Ghost promotion. Read the board first: whether this score is worth racing
     * against depends on the current top of
    ...
    // The job has served its purpose, and the state blob is the largest thing in the
    // table.
    ...
    /*
     * Release the lease and record why, rather than rejecting the run. A verifier
     * fault is not the player's fault, so
     * the job stays claimable and the slice budget bounds how many times it can
