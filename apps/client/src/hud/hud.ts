...
export class Hud {
  ...
  update(snapshot: RenderSnapshot, now: number): void {
    ...
    this.timerText.textContent = `${Math.floor(seconds / 60)}:${String(seconds % 60).padStart(2, '0')}`;
    ...
    } else this.streakText.dataset.visible = 'false';
    ...
    } else this.downNotice.dataset.visible = 'false';
    ...
    while (this.feedEntries.length > 0 && this.feedEntries[0]!.until < now)
      this.feedEntries.shift()!.node.remove();
  }
