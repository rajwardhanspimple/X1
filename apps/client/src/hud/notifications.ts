...
export function mountNotifications(root: HTMLElement): () => void {
  ...
  const render = (rows: PlayerNotification[]) => {
    ...
    badge.textContent = unread ? `Notifications (${unread})` : 'Notifications';
    badge.addEventListener('click', async () => {
      for (const row of rows.filter((item) => !item.read_at))
        await supabase?.rpc('mark_read', { notification_id: row.id });
      await load();
    });
    root.append(badge);
  };
