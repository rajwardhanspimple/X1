...
export class OperatorDashboard {
  ...
  async load(): Promise<void> {
    if (!supabase) {
      this.root.textContent = 'Operator console is offline.';
      return;
    }
    const { data, error } = await supabase
      .from('verifier_rejection_rates')
      .select('*')
      .order('client_version');
    if (error) {
      this.root.textContent = `Unable to load verification health: ${error.message}`;
      return;
    }
  ...
  render(): void {
    ...
    const empty = emptyState(this.rows);
    if (empty) {
    ...
    table.innerHTML =
      '<thead><tr><th>Client version</th><th>Device class</th><th>Reason</th><th>Rate</th></tr></thead>';
    ...
    for (const row of this.rows) {
      ...
      for (const value of [
        row.client_version,
        row.device_class,
        row.rejection_reason ?? 'All',
        formatRate(row.rate, row.total),
      ]) {
  ...
  async invalidateRun(runId: string, reason: string): Promise<void> {
    ...
    const { error } = await supabase.rpc('invalidate_run', {
      run_id: runId,
