...
export class AccountScreen {
  ...
  private failureHtml(): string {
    ...
    if (!f) return '';
    const retry = f.retryable
      ? '<button type="button" data-act="retry" class="btn-secondary">Try again</button>'
      : '';
    return `<div class="account-error" role="alert"><p>${this.esc(f.message)}</p>${retry}</div>`;
  }
  private render(): void {
  ...
  private renderOverview(): void {
    ...
    let actions = '';
    if (!unavailable && this.state.status === 'guest')
      actions =
        '<button type="button" data-act="upgrade" class="btn-primary">Save my progress</button><button type="button" data-act="signin" class="btn-secondary">I already have an account</button>';
    else if (!unavailable && this.state.status === 'account')
      actions =
        '<button type="button" data-act="profile" class="btn-primary">Profile</button><button type="button" data-act="signout" class="btn-secondary">Sign out</button>';
    else if (!unavailable && this.state.status === 'offline')
      actions =
        '<button type="button" data-act="guest" class="btn-primary">Play as guest</button><button type="button" data-act="signin" class="btn-secondary">Sign in</button>';
    const providers = this.state.providers.length
    ...
    this.bind();
    const host = this.root.querySelector<HTMLElement>('.recent-runs');
    if (host) void renderRecentRuns(host);
  }
  private methodTabs(): string {
    return SIGN_IN_METHODS.map(
      (m) =>
        `<button type="button" data-method="${m}" class="account-tab${m === this.method ? ' is-active' : ''}">${this.esc(describeMethod(m))}</button>`,
    ).join('');
  }
  private renderUpgrade(): void {
    const form =
      this.method === 'password'
        ? `<label>Email<input type="email" data-field="email" autocomplete="email" value="${this.esc(this.carriedEmail)}"></label><label>Password<input type="password" data-field="password" autocomplete="new-password"></label><button type="button" data-act="do-upgrade" class="btn-primary"${this.state.busy ? ' disabled' : ''}>Save progress</button>`
        : this.method === 'magic-link'
          ? `<label>Email<input type="email" data-field="email" autocomplete="email" value="${this.esc(this.carriedEmail)}"></label><button type="button" data-act="do-upgrade" class="btn-primary"${this.state.busy ? ' disabled' : ''}>Send me a link</button>`
    ...
    this.root.innerHTML = `<div class="screen-panel"><h2>Save your progress</h2><p class="account-note">Your scores and unlocks stay exactly as they are. This only adds a way to sign back in.</p><div class="account-tabs">${this.methodTabs()}</div><div class="account-form">${form}</div>${this.failureHtml()}<button type="button" data-act="overview" class="btn-quiet">Back</button></div>`;
    this.bind();
  }
  private renderSignIn(): void {
    const form =
      this.method === 'password'
        ? `<label>Email<input type="email" data-field="email" autocomplete="email" value="${this.esc(this.carriedEmail)}"></label><label>Password<input type="password" data-field="password" autocomplete="current-password"></label><button type="button" data-act="do-signin" class="btn-primary"${this.state.busy ? ' disabled' : ''}>Sign in</button>`
        : this.method === 'magic-link'
          ? `<label>Email<input type="email" data-field="email" autocomplete="email" value="${this.esc(this.carriedEmail)}"></label><button type="button" data-act="do-signin" class="btn-primary"${this.state.busy ? ' disabled' : ''}>Send me a link</button>`
    ...
    const warning =
      this.state.status === 'guest'
        ...
        : '';
    this.root.innerHTML = `<div class="screen-panel"><h2>Sign in</h2>${warning}<div class="account-tabs">${this.methodTabs()}</div><div class="account-form">${form}</div>${this.failureHtml()}<button type="button" data-act="overview" class="btn-quiet">Back</button></div>`;
    this.bind();
  }
  private renderConfirmSwitch(): void {
    this.root.innerHTML = `<div class="screen-panel"><h2>Leave this progress behind?</h2><p class="account-warn">You are playing as a guest. Signing in to another account will leave these scores and unlocks behind.</p><label>Type ${CONFIRM_WORD} to continue<input type="text" data-field="confirm" autocomplete="off"></label><div class="screen-actions"><button type="button" data-act="confirm-switch" class="btn-danger">Sign in anyway</button><button type="button" data-act="overview" class="btn-secondary">Keep my progress</button></div></div>`;
    this.bind();
  }
  ...
  private bind(): void {
    for (const tab of this.root.querySelectorAll<HTMLElement>('[data-method]'))
      tab.addEventListener('click', () => {
        ...
        if (email) this.carriedEmail = email;
        this.render();
      });
  ...
  private async act(action: string): Promise<void> {
    ...
    if (action === 'do-upgrade') {
      ...
      else if (this.method === 'magic-link') await this.auth.upgradeWithMagicLink(email);
      else await this.auth.upgradeWithProvider(this.method);
    ...
    if (action === 'do-signin') {
      ...
      else await this.auth.signInWithProvider(this.method);
      if (!this.state.failure && this.state.status === 'account') this.panel = 'overview';
      return this.render();
    }
  }
