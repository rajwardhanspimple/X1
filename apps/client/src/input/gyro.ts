...
export class GyroAimProvider {
  ...
  private readonly orientationWindow: OrientationWindow | null = typeof window === 'undefined' ? null : window;

  constructor(private readonly onLook: (yawTurns: number, pitchTurns: number) => void, private readonly explain: (message: string) => void = () => {}) {}
  ...
  async enable(): Promise<boolean> {
    ...
    if (permission !== 'granted') {
      this.enabled = false;
      this.last = null;
      this.explain('Gyroscope access was denied. Enable motion access in your device settings, then try again.');
      return false;
    }
  ...
  private async requestPermission(): Promise<OrientationPermission> {
    const eventType = this.orientationWindow?.DeviceOrientationEvent as (typeof DeviceOrientationEvent & { requestPermission?: () => Promise<OrientationPermission> }) | undefined;
    if (!eventType?.requestPermission) return this.orientationWindow ? 'granted' : 'denied';
    try { return await eventType.requestPermission(); } catch { return 'denied'; }
  }
  ...
  private readonly handle = (event: Event): void => {
    ...
    if (this.last && current.alpha !== null && this.last.alpha !== null && current.beta !== null && this.last.beta !== null) {
