...
import { clampLayout, loadTouchState, saveTouchState, DEFAULT_TOUCH_LAYOUT, DEFAULT_TOUCH_PREFERENCES } from './touch-layout.js';

describe('touch layout persistence', () => {
  const area = { width: 1000, height: 500 };
  beforeEach(() => localStorage.clear());
  it('clamps positions and preserves the 44px minimum', () => {
    const layout = clampLayout({ fire: { ...DEFAULT_TOUCH_LAYOUT.fire, x: 2, y: -1, width: 0.001, height: 0.001 } }, area);
  ...
  it('round trips layout and preferences through local storage', () => {
    const state = { layout: DEFAULT_TOUCH_LAYOUT, preferences: { ...DEFAULT_TOUCH_PREFERENCES, gyroEnabled: true } };
