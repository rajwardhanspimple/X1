...
export class TouchAdapter {
  ...
  addLookDelta(yawTurns: number, pitchTurns: number): void {
    if (!this.enabled) return;
    this.yawAccum += yawTurns;
    this.pitchAccum += pitchTurns;
  }
  private bindStick(): void {
    ...
    const onMove = (event: PointerEvent) => {
      ...
      this.callbacks.onStickMove?.({
        ...
        dx: event.clientX - this.stick.originX,
        dy: event.clientY - this.stick.originY,
      });
      event.preventDefault();
    };
  ...
  private bindLook(): void {
    ...
    const onMove = (event: PointerEvent) => {
      ...
      const scale =
        ...
        (this.isActionHeld('aim') ? this.settings.adsSensitivityScale : 1);
  ...
  bindButton(element: HTMLElement, action: TouchAction): void {
    ...
    const release = (event: PointerEvent) => {
      ...
      this.callbacks.onHaptic?.('release');
