...
export function hasOperatorRole(): boolean {
  return supabase?.auth.getSession().then(({ data }) => data.session?.user.app_metadata?.role === 'operator') as unknown as boolean;
}
...
export async function mountOperatorDashboard(host: HTMLElement): Promise<() => void> {
  ...
  const dashboard = new OperatorDashboard(host); await dashboard.load();
  const notificationRoot = document.createElement('div'); host.append(notificationRoot); const notifications = mountNotifications(notificationRoot);
  return () => { notifications(); host.replaceChildren(); };
}
