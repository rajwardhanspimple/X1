...
describe('tamper detection', () => {
  ...
  it('an edited movement input changes the checkpoint hashes', () => {
    const log = recordLog(180);
    // Frame 5, not 70: by frame 70 the player is already moving that way, so the edit was a no-op.
    const frames = log.frames.map((f, i) => (i === 5 ? { ...f, moveX: fx.FX_ONE } : f));
    const result = replay({ log: { ...log, frames }, content });
    expect(result.mismatchTick).not.toBeNull();
  });
  ...
  it('an edited fire input changes the checkpoint hashes', () => {
    const log = recordLog(180);
    // Frame 10, not 60: by frame 60 the weapon is on cooldown from the previous shot, so the edit was a no-op.
    const frames = log.frames.map((f, i) =>
      i === 10 ? { ...f, buttons: f.buttons | Buttons.Fire } : f,
    );
    const result = replay({ log: { ...log, frames }, content });
    expect(result.mismatchTick).not.toBeNull();
  });
